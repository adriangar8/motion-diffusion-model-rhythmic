...
def get_process_file(conversion_repo, example_joints_path=None):
    ...
    if os.path.exists(momask_path):
        ...
        sys.path.insert(0, conversion_repo)
        # ensure project root is in sys.path so data_loaders package is importable
...
def smpl_to_joints_22(smpl_poses, smpl_trans, smpl_scaling, smpl_model):
    ...
    for start in range(0, T, batch_size):
        ...
        # apply AIST++ convention
        joints = joints * scaling + trans_batch.unsqueeze(1)
...
def main():
    ...
    parser.add_argument('--conversion_repo', type=str,
                        default='./data_loaders/humanml',
                        help='Path to cloned MoMask or HumanML3D repo (default: bundled copy)')
    ...
    np.savez(os.path.join(args.aist_dir, 'processed_stats.npz'),
             mean=mean, std=std)
