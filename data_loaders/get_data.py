...
def get_dataset(name, num_frames, split='train', hml_mode='train', abs_path='.', fixed_len=0, 
                device=None, autoregressive=False, cache_path=None, humanml_dir=None): 
    DATA = get_dataset_class(name)
    if name in ["humanml", "kit"]:
        kwargs = dict(split=split, num_frames=num_frames, mode=hml_mode, abs_path=abs_path, fixed_len=fixed_len, 
                     device=device, autoregressive=autoregressive)
        if humanml_dir is not None and humanml_dir != '':
            kwargs['humanml_dir'] = humanml_dir
        dataset = DATA(**kwargs)
    else:
...
def get_dataset_loader(name, batch_size, num_frames, split='train', hml_mode='train', fixed_len=0, pred_len=0, 
                       device=None, autoregressive=False, humanml_dir=None):
    dataset = get_dataset(name, num_frames, split=split, hml_mode=hml_mode, fixed_len=fixed_len, 
                device=device, autoregressive=autoregressive, humanml_dir=humanml_dir)
